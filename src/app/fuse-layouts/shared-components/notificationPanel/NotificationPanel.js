...
import { dismissAll, dismissItem, selectNotifications } from './store/dataSlice';
...
function NotificationPanel(props) {
  ...
  useEffect(() => {
    // ws.sendMessage('notification/findAll');
  }, [ws]);

  useEffect(() => {
    notifications.forEach((item) => {
      const { id: key, message, options = {}, dismissed = false } = item;
      if (options.type === 'message' && location.pathname === '/apps/chat') {
  ...
  }, [notifications, closeSnackbar, enqueueSnackbar, dispatch, location.pathname]);
