...
import { closeNewTodoDialog, closeEditTodoDialog } from './store/todosSlice';
...
function TodoDialog(props) {
  ...
  function onSubmit(data) {
    if (todoDialog.type === 'new') {
      const newTodo = { ...data };
      delete newTodo.id;
      ws.sendMessage('todo/add', newTodo);
    } else {
      ws.sendMessage('todo/update', { ...todoDialog.data, ...data });
    }
    closeTodoDialog();
  }

  function handleRemove() {
    ws.sendMessage('todo/remove', formId);
    dispatch(closeTodoDialog());
  }
