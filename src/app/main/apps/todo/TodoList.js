...
function TodoList(props) {
  ...
  useEffect(() => {
    ...
    if (todos) {
      const { filterHandle, labelHandle } = params;

      let newTodos = [...todos];

      if (filterHandle) {
        newTodos = newTodos.filter((t) => t[filterHandle]);
      }

      if (labelHandle) {
        const findedLabel = labels.find((l) => l.handle === labelHandle);
        if (findedLabel) {
          newTodos = newTodos.filter((t) => t.labels.includes(findedLabel.id));
        }
      }

      setFilteredData(
        _.orderBy(
          getFilteredArray(newTodos, searchText),
          [orderBy],
          [orderDescending ? 'desc' : 'asc']
        )
      );
    }
  }, [todos, searchText, orderBy, orderDescending, params, labels]);
