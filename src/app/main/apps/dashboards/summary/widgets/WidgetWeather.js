  ...
  return (
    <Paper className="w-full rounded-20 shadow flex flex-col justify-between">
      ...
      <div className="flex items-center justify-center p-20 pb-32">
        <img
          color="action"
          title={props.widget.locations[props.widget.currentLocation].text}
          alt={props.widget.locations[props.widget.currentLocation].text}
          src={props.widget.locations[props.widget.currentLocation].icon}
        />
      ...
      <div className="flex justify-between items-center p-16">
        ...
        <div className="flex items-center">
          ...
          <Typography className="mx-4 font-semibold">
            %{props.widget.locations[props.widget.currentLocation].humidity}
          </Typography>
        </div>
      </div>
      <Divider />
        ...
        {props.widget.locations[props.widget.currentLocation].next5Days.map((day) => (
          <div className="flex items-center justify-between w-full py-16 px-24" key={day.name}>
            <Typography className="text-15 font-medium">
              {new Date(day.name).toLocaleString('en-EN', { weekday: 'long' })}
            </Typography>
            <div className="flex items-center">
              <img
                className="meteocons text-40 ltr:mr-8 rtl:ml-8"
                src={day.icon}
                alt={day.text}
                title={day.text}
              />
