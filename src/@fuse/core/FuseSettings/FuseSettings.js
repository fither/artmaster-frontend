...
import { useCallback, useMemo, memo, useEffect, useContext } from 'react';
...
function FuseSettings(props) {
  ...
  const handleUpdate = useDebounce((newSettings) => {
    if (user.role === 'guest') {
      ...
      ws.sendMessage('user/updateUserSettings', newSettings);
      // dispatch(updateUserSettings(newSettings, ws));
    }
  }, 300);
